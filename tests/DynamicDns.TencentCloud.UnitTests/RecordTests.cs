...
namespace DynamicDns.TencentCloud.UnitTests
{
    ...
    public class RecordTests:TestBase
    {
        ...
        {
            Assert.True(await DomainRecordUtil.AddOrUpdateAsync(new CreateRecordRequestModel(TestConfigData.QueryDomain, "test" + new Random(DateTime.Now.Millisecond).Next(100, 10000), "TXT", "abc")));
        }
    }
}